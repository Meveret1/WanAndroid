...
class DynamicListAdapter(private val list: MutableList<DataX>, hasMore: Boolean) :
        ...
        if (holder is RootHolder) {
            if (position < list.size) {
                ...
                holder.collected.setOnClickListener {
                    if (jsonstr.collect) {
                        Constant.retrofithelper.removeCollect(Constant.oldcookie,jsonstr.id).enqueue(object : Callback<JsonData> {
